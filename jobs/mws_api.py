...
class responseProcessor:
	...
	def mwsResponseProcessor(responses):
		if responses:
			dataDict = {
			 		'ReportType':[], 
			...
			for response in responses:
				...
				dataDict[RequestId].append(response.ReportRequestInfo.RequestId.getvalue('RequestId'))
		...
		#Adding Data to mwsProcessLog Tables
			...
			Session = sessionmaker(bind=engine)
			session = Session()
			cData = (session.query(ChannelAccess).join(Channel,Channel.id == ChannelAccess.channel_id).all())
			session.close()	